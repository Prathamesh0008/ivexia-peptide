...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[#eeeeee] bg-white">
      {/* Top Row */}
      <div className="mx-auto flex h-[82px] w-full max-w-[1500px] items-center px-6 lg:px-10">
        <Link href="/" className="flex w-[315px] shrink-0 items-center">
          <Image
            src="/Ivexia_Peptide.png"
            alt="Ivexia Peptide"
            width={320}
            height={72}
            priority
            className="h-auto w-[305px] object-contain"
          />
        </Link>

        <div className="hidden flex-1 justify-center lg:flex">
          <div className="relative flex h-[46px] w-full max-w-[740px] items-center rounded-[7px] border border-[#cfcfcf] bg-white px-5">
            <button type="button" onClick={submitSearch} aria-label="Search">
              <Search size={18} strokeWidth={2} className="text-[#666]" />
            </button>
            ...
            <input
              ...
              className="ml-3 w-full bg-transparent text-[14px] text-black outline-none placeholder:text-[#777]"
              placeholder={t("search", "")}
            />
        ...
        <div className="hidden w-[315px] shrink-0 items-center justify-end gap-6 lg:flex">
          <LanguageMenu
            languageOpen={languageOpen}
            selectedLanguage={selectedLanguage}
            toggleLanguageOpen={() => setLanguageOpen((current) => !current)}
          ...
          {sessionEmail ? (
            <div ref={profileRef} className="relative shrink-0">
              <button
                type="button"
                onClick={() => setProfileOpen((prev) => !prev)}
                className="flex items-center gap-2 whitespace-nowrap text-[14px] font-normal text-black hover:text-[#F04423]"
              >
                <AccountIcon size={22} />
                Profile
                <ChevronDown size={13} />
              ...
              {profileOpen && (
                <div className="absolute right-0 top-full z-[9999] mt-3 w-[180px] overflow-hidden rounded-xl border border-[#E5E5E5] bg-white shadow-xl">
                  <Link
                    href="/account"
                    onClick={() => setProfileOpen(false)}
                    className="block px-4 py-3 text-sm hover:bg-[#FFF2EF] hover:text-[#F04423]"
                  >
                    My Profile
                  </Link>
          ...
          <Link
            ...
            className="flex items-center gap-2 whitespace-nowrap text-[14px] font-normal text-black hover:text-[#F04423]"
          >
            <CartIcon itemCount={itemCount} size={22} />
            {t("myCart", "My Cart")}
          </Link>
        </div>
      ...
      {/* Bottom Row */}
      <nav className="hidden h-[54px] lg:block">
        <div className="mx-auto flex h-full w-full max-w-[1500px] items-center justify-between px-[135px]">
          <div className="flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.key}
                href={link.href}
                className="whitespace-nowrap text-[14px] font-normal text-[#111] transition hover:text-[#F04423]"
              >
                {t(link.key, link.fallback)}
              </Link>
            ))}
    ...
    <div className={mobile ? "relative w-fit" : "relative"}>
      <button
        ...
        className={
          mobile
            ? "flex items-center gap-2 rounded-md border border-[#D8D8D8] px-3 py-2 text-sm font-medium text-[#06101F]"
            : "flex items-center gap-1 text-[13px] font-normal text-black hover:text-[#F04423]"
        }
      >
        {selectedLanguage.code}
        <ChevronDown size={mobile ? 16 : 13} strokeWidth={2} />
      </button>
...
function AccountIcon({ size }: { size: number }) {
  return (
    ...
    >
      <circle cx="13" cy="7" r="3.65" stroke="currentColor" strokeWidth="1.45" />
      <path
        d="M5.05 21.15v-1.3c0-3.15 3.55-5.45 7.95-5.45s7.95 2.3 7.95 5.45v1.3c-2.05.55-4.75.85-7.95.85s-5.9-.3-7.95-.85Z"
        stroke="currentColor"
        strokeWidth="1.45"
...
function CartIcon({
  ...
  return (
    <span className="relative">
      ...
      {itemCount > 0 && (
        <span className="absolute -right-2 -top-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#F04423] px-1 text-[10px] font-semibold text-white">
          {itemCount}
        </span>
      )}
    </span>
